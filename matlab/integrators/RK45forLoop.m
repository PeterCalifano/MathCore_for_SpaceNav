function [tGrid, xState, info] = RK45forLoop(fDyn, x0, h0, t0, Nsteps, order)
%% PROTOTYPE
...
% -------------------------------------------------------------------------------------------------------------
%% CHANGELOG
% 08-08-2023    Pietro Califano     RK45 integrator compatible with casadi
...
for idN = 1:Nsteps
    ...
    if notlastStep
        ...
        tGridTemp = zeros(Nsteps, 1);
        
        if not(symbFlag)
            tGridTemp = zeros(Nsteps, 1);
            xStateTemp = zeros(Ndim, Nsteps);
        end
    end
