function [shellList, nodesArrayCell, cornersIDs] = GenIcosahedronDiscrSphere(densityArray, sphereRadii, path2Icomain)
%% PROTOTYPE
% [shellList, nodesArrayCell, cornersIDs] = GenIcosahedronDiscrSphere(densityArray, sphereRadii, path2Icomain)
% -------------------------------------------------------------------------------------------------------------
%% DESCRIPTION
...
computeNumNodes = @(densityValue) 40*densityValue*(densityValue - 1) + 12; 
...
if strcmp(computer, 'PCWIN64')
    programName = which("icomain.exe");
elseif strcmp(computer, 'GLNXA64')
    programName = "icomain";
end

if not(exist('path2Icomain', 'var'))
    path2Icomain = fullfile(programName);
else
    path2Icomain = fullfile(path2Icomain, programName);
end

% Static allocation
Nshells = length(densityArray);
filename = strings(Nshells, 1);
shellList = strings(Nshells, 1);
Nnodes = zeros(Nshells, 1);
cornersIDs = zeros(Nshells, 12);

nodesArrayCell = cell(1, Nshells);
...
for shellID = 1:Nshells
    ...
    if strcmp(computer, 'PCWIN64')
        ...
        Nnodes(shellID) = computeNumNodes(round(densityArray(shellID)));
        fprintf('\nNumber of nodes = %4.0f', Nnodes(shellID));
        NewShellName = "Shell" + num2str(shellID);
        shellList(shellID) = NewShellName;

        filename(shellID) = "nodesArray_density" + num2str(densityArray(shellID)) + ".dat";

        % Check if nodedensity map already exist or not
        if ~exist(filename(shellID) , "file")
            %   Write batch file to call icomain
            command_string = "ECHO   "   + num2str(densityArray(shellID) )  + "  | " + '"' + path2Icomain + '"';
    ...
    nodesArrayCell{shellID} = sphereRadii(shellID) * NodesArrayTmp(:, 2:end);

    % Create corners ids
    nCorners = 0:11;
    cornersIDs(shellID, :) = (Nnodes(shellID) - nCorners);
